...
class MigrateLegacyAliado extends Command
{
    ...
    {
        $aliadoKey = $this->argument('aliado');
        if (!isset($this->dbsMap[$aliadoKey])) {
            $this->error("Aliado '$aliadoKey' no definido en el mapa.");
            return;
        }
        $this->dbs = [$this->dbsMap[$aliadoKey] => $aliadoKey];
        ...
        foreach ($this->dbs as $db => $key) {
            ...
            while (true) {
                ...
                foreach ($rows as $r) {
                    ...
                    // SOLO si el contrato está vigente
